...
public class BaseAction {
	...
	public void tap( AndroidElement element){
        tap(element.getLocation().getX(),element.getLocation().getY());
    }
	...
	//根据控件坐标实行长按操作
	    public void longtap( int x, int y) {
	    	...
	    	LongPressOptions longPressOptions = LongPressOptions.longPressOptions();
			longPressOptions.withPosition(PointOption.point(x, y));
			action.longPress(longPressOptions).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(1))).release().perform();
	    }
	...
	//长按并拖动到坐标点
	public void dragAndSlide1(AndroidElement element,int x,int y) {
